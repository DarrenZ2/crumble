package
{
	...
	public final class WindWidget extends Sprite implements IAnimatable
	{
		...
		{
			var moveTouches:Vector.<Touch> = event.getTouches(Game.service.foreground, TouchPhase.MOVED);
			if (moveTouches.length == 1) {
				var touch:Touch = moveTouches[0];
				var p0:Point = touch.getPreviousLocation(Game.service.foreground.parent);
				p0.x -= stage.stageWidth/2;
				p0.y -= stage.stageHeight/2;
				var dp:Point = touch.getMovement(Game.service.foreground.parent);
				var tangent:Vec2 = new Vec2(-p0.y, p0.x);
				var flux:Number = tangent.dot(Vec2.fromPoint(dp));
				_currentAngle += flux*0.000025;
				_currentAngle = Math.max(Math.min(_currentAngle, maxAngle), minAngle);
			}
			
		}
		
		public function WindWidget()
		{
//			accelerometer = new Accelerometer();
//			accelerometer.addEventListener(AccelerometerEvent.UPDATE, onAccelerometerUpdate);
			Game.service.foreground.parent.addEventListener(TouchEvent.TOUCH, onTouch);
			...
			// add a button to confirm the setting when the windWidget widget is visible
